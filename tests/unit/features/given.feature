Feature: Reusable `given` steps
  ...
  Scenario: Deploy with all optionals
    Given I add model 'test'
    Given I deploy 'slurmctld' in model 'test' from channel 'latest/edge' on base 'ubuntu@24.04'
  ...
  Scenario: Deploy local with all optionals
    Given I add model 'test'
    Given I deploy 'slurmctld' from a local charm located at '/tmp/fake.charm' in model 'test' on base 'ubuntu@24.04'
